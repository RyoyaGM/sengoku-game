                    ...
                    {event.choices && event.choices[daimyoId] ? (
                        <div className="space-y-4">
                            ...
                            {event.choices[daimyoId].map((choice) => (
                                <button key={choice.id} onClick={() => onSelect(choice)} className="w-full p-4 rounded border border-stone-500 bg-stone-700 hover:bg-stone-600 transition-all text-left group hover:border-yellow-500 relative overflow-hidden">
        ...
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
            <div className="bg-stone-800 text-white p-6 rounded-xl border border-teal-500 shadow-2xl w-full max-w-sm">
                ...
                <div className="text-center mb-4">
                    <div className="text-4xl font-mono font-bold mb-2">{currentVal} <span className="text-sm">石</span></div>
                    <div className="text-sm text-stone-300">{mode === 'buy' ? `費用: ${cost}金` : `売上: ${gain}金`}</div>
                </div>
    ...
    return ( 
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
            <div className="bg-stone-800 text-white p-6 rounded-xl border border-purple-500 shadow-2xl w-full max-w-sm">
                ...
                <div className="text-center mb-4">
                    <div className="text-4xl font-mono font-bold mb-2 text-yellow-400">{amount}金</div>
                    <div className="text-sm text-stone-300">名声期待値: +{fameGain}</div>
                </div>
                ...
                <div className="flex gap-4">
                    <button onClick={onCancel} className="flex-1 py-2 rounded bg-stone-600 font-bold">やめる</button>
                    <button onClick={() => onConfirm(target, amount, fameGain)} disabled={currentGold < amount} className="flex-1 py-2 rounded font-bold bg-purple-600 hover:bg-purple-500 text-white disabled:opacity-50 flex items-center justify-center gap-1">献上実行 <Zap size={14} className="text-yellow-400"/> 1</button>
                </div>
            </div>
        </div> 
    ); 
...
export const NegotiationScene = ({ targetDaimyoId, targetDaimyo, isAllied, onConfirm, onCancel, isPlayerAinu }) => ( 
    <div className="fixed inset-0 z-50 bg-black/90 flex flex-col items-center justify-center p-4 animate-fade-in">
        <div className="flex flex-col items-center mb-8 z-10">
            <div className={`w-3 h-32 rounded-full ${targetDaimyo.color} flex items-center justify-center border-4 border-white shadow-lg mb-4`}><span className="text-4xl font-bold text-white">{targetDaimyo.name[0]}</span></div>
        ...
        <div className="w-full max-w-2xl bg-stone-900 border-2 border-stone-600 rounded-xl p-6 shadow-2xl z-10">
            ...
            <div className="grid grid-cols-2 gap-4">
                {isAllied ? (
                    <><button onClick={() => onConfirm('request_aid')} className="p-3 bg-indigo-900/50 hover:bg-indigo-800 border border-indigo-500 rounded text-white font-bold flex justify-between items-center">援助要請 <span className="flex items-center gap-1 text-xs bg-black/30 px-2 rounded"><Zap size={10}/>1</span></button>
                    <button onClick={() => onConfirm('break_alliance')} className="p-3 bg-red-900/50 hover:bg-red-800 border border-red-500 rounded text-white font-bold flex justify-between items-center">同盟破棄 <span className="flex items-center gap-1 text-xs bg-black/30 px-2 rounded"><Zap size={10}/>1</span></button></>
                ) : (
        ...
        <div className="fixed inset-0 z-50 bg-black/90 text-white flex flex-col items-center justify-center p-4 animate-fade-in">
            <div className="flex justify-between w-full max-w-4xl items-center mb-8">
                <div className="flex flex-col items-center">
                    <div className={`w-24 h-24 rounded-full ${attackerDaimyo.color} flex items-center justify-center border-4 border-white shadow-[0_0_20px_rgba(255,0,0,0.5)] mb-2 relative`}><span className="text-3xl font-bold">{attackerDaimyo.name[0]}</span><div className="absolute -bottom-2 bg-red-700 px-2 rounded-full text-xs font-bold border border-red-400">攻撃軍</div></div>
                ...
                <div className="flex flex-col items-center justify-center"><div className="text-6xl font-black italic text-yellow-500 mb-2">VS</div><div className="text-xl font-bold text-stone-400">Round {round}/10</div></div>
                <div className="flex flex-col items-center">
                    <div className={`w-24 h-24 rounded-full ${defenderDaimyo.color} flex items-center justify-center border-4 border-white shadow-[0_0_20px_rgba(0,0,255,0.5)] mb-2 relative`}><span className="text-3xl font-bold">{defenderDaimyo.name[0]}</span><div className="absolute -bottom-2 bg-blue-700 px-2 rounded-full text-xs font-bold border border-blue-400">防衛軍</div></div>
            ...
            <div className="w-full max-w-2xl bg-stone-900/80 border border-stone-600 rounded-lg h-48 overflow-y-auto p-4 mb-6 font-mono text-sm">{logs.slice().reverse().map((l, i) => (<div key={i} className={`mb-1 ${l.includes('勝利') ? 'text-yellow-400 font-bold text-lg' : 'text-stone-300'}`}>{l}</div>))}</div>
...
export const GameOverScreen = ({ gameState, onRestart }) => ( 
    <div className="absolute inset-0 bg-black/90 flex flex-col items-center justify-center z-50">
        <div className="text-center animate-bounce mb-6">{gameState === 'won' ? <><Trophy size={80} className="text-yellow-400 mx-auto mb-4" /><h2 className="text-5xl font-bold text-yellow-400">天下統一！</h2></> : <><Skull size={80} className="text-gray-500 mx-auto mb-4" /><h2 className="text-5xl font-bold text-gray-400">落城...</h2></>}</div>
...
export const RecruitSelector = ({ maxRecruitable, costPerTroop, onConfirm, onCancel }) => { 
    const initialAmount = Math.min(maxRecruitable, Math.max(10, Math.floor(maxRecruitable * 0.2))); 
    const [amount, setAmount] = useState(initialAmount); 
    const goldCost = Math.ceil(amount * costPerTroop.gold); 
    const riceCost = Math.ceil(amount * costPerTroop.rice); 
    return ( 
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4"> 
            <div className="bg-stone-800 text-white p-6 rounded-xl border border-teal-600 shadow-2xl w-full max-w-sm"> 
                <h3 className="text-xl font-bold mb-4 flex items-center gap-2 text-teal-400"> <Users size={24} /> 徴兵 </h3> 
                <div className="text-center mb-2"> <div className="text-sm text-stone-400 mb-1">雇用人数</div> <div className="text-4xl font-mono font-bold text-white">{amount} <span className="text-lg font-normal text-stone-500">人</span></div> </div> 
                <div className="bg-stone-900 rounded p-3 mb-6 grid grid-cols-2 gap-4 text-center border border-stone-700"> <div> <div className="text-xs text-yellow-400 font-bold">必要資金</div> <div className="font-mono text-xl">{goldCost}</div> </div> <div> <div className="text-xs text-green-400 font-bold">必要兵糧</div> <div className="font-mono text-xl">{riceCost}</div> </div> </div> 
                <input type="range" min="10" max={maxRecruitable} step="10" value={amount} onChange={(e) => setAmount(parseInt(e.target.value))} className="w-full h-2 bg-stone-600 rounded-lg appearance-none cursor-pointer mb-2 accent-teal-500" /> 
                <div className="flex justify-between text-xs text-stone-500 mb-6"> <span>10</span> <span>{maxRecruitable}</span> </div> 
                <div className="flex gap-4"> <button onClick={onCancel} className="flex-1 py-2 bg-stone-600 rounded font-bold hover:bg-stone-500">キャンセル</button> <button onClick={() => onConfirm(amount)} className="flex-1 py-2 bg-teal-700 hover:bg-teal-600 rounded font-bold flex items-center justify-center gap-1"> 雇用 <Zap size={14} className="text-yellow-400"/> </button> </div> 
            </div> 
        </div> 
    ); 
};

export const ReinforcementRequestModal = ({ attacker, defender, potentialAllies, selfReinforcements, relations, onConfirm }) => { 
    return ( 
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
            <div className="bg-stone-800 border-2 border-red-600 p-6 rounded-lg max-w-lg w-full text-stone-100 shadow-2xl animate-fade-in">
                ...
                <div className="mb-4">
                    <p className="text-lg"><span className={`font-bold ${DAIMYO_INFO[attacker.ownerId]?.color || 'bg-gray-500'} px-2 py-0.5 rounded`}>{DAIMYO_INFO[attacker.ownerId]?.name || attacker.ownerId}</span>軍が</p>
                ...
                <div className="space-y-4 mb-6 max-h-80 overflow-y-auto">
                    {selfReinforcements && selfReinforcements.length > 0 && (
                        <div className="mb-4">
                            <h3 className="font-bold mb-2 border-b border-stone-600 pb-1 text-teal-400">近隣の自国部隊 (即時合流)</h3>
                            <div className="space-y-2">{selfReinforcements.map(prov => { const sendable = Math.max(0, prov.troops - 100); if (sendable <= 0) return null; return ( <button key={prov.id} onClick={() => onConfirm(prov.id, 'self')} className="w-full flex justify-between items-center p-3 bg-teal-900/40 hover:bg-teal-800/60 rounded border border-teal-700 transition-colors"><div className="flex items-center gap-2"><span>{prov.name}</span><span className="text-xs text-teal-200">から援軍</span></div><div className="text-sm font-mono font-bold text-teal-200">+{sendable}人</div></button> ); })}</div>
                        </div>
                    )}
                    <div>
                        <h3 className="font-bold mb-2 border-b border-stone-600 pb-1 text-yellow-400">同盟国へ要請 (確率)</h3>
                        <div className="space-y-2">{potentialAllies.length === 0 ? (<p className="text-gray-500 text-sm">要請可能な同盟国がありません...</p>) : (potentialAllies.map(ally => { const rel = relations[defender.ownerId]?.[ally.id] || 50; const successRate = rel >= 80 ? '高' : rel >= 40 ? '中' : '低'; return ( <button key={ally.id} onClick={() => onConfirm(ally.id, 'ally')} className="w-full flex justify-between items-center p-3 bg-stone-700 hover:bg-stone-600 rounded border border-stone-500 transition-colors"><div className="flex items-center gap-2"><div className={`w-3 h-3 rounded-full ${DAIMYO_INFO[ally.id]?.color || 'bg-gray-500'}`}></div><span>{DAIMYO_INFO[ally.id]?.name}</span><span className="text-xs text-stone-400">(関係: {rel})</span></div><div className="text-xs text-yellow-400">成功率: {successRate}</div></button> ); }))}</div>
                    </div>
                </div>
                <div className="flex justify-end gap-3"><button onClick={() => onConfirm(null, null)} className="px-4 py-2 bg-stone-600 hover:bg-stone-500 rounded font-bold">援軍なしで迎撃</button></div>
            </div>
        </div> 
    ); 
};

// --- 追加コンポーネント: 投資額決定用モーダル ---
